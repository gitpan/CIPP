...
sub Exception {
	...
	Log ("EXC", "TYPE=$type, MESSAGE=$message");
...
sub Log {
	...
	if ( not $CIPP_Exec::apache_mod ) {
		...
		open (cipp_LOG_FILE, ">> $filename") or
			die "$throw\tKonnte $filename nicht zum Schreiben öffnen";
		if ( ! print cipp_LOG_FILE "$time\t$msg\n" ) {
			close cipp_LOG_FILE;
			die "$throw\tKonnte nicht in $filename schreiben";
		}
		close cipp_LOG_FILE;
	} else {

...
# TODO
#
# - bei General Exception prüfen ob HTTP Header schon generiert
...
sub Generate_Database_Code {
	...
	if ( $self->{result_type} eq 'cipp-module' ) {
		# Eines eines Moduls: 1 zurückgeben
		$self->{output}->Write ( "\n}\n1;\n" );
		return;
	}
...
sub Format_Debugging_Source {
	...
	# Erstmal eine Liste der Fehler erzeugen, später kommt dann
...
#
	...
	# Schlüssel für Include-Cache: Name des Objektes + Flag,
...
#	}
	...
	# Fehler auf, der Stack muß das SQL Statement aber schon
...
#
	...
	if ( not $apache_mod ) {
		# Projektname muß raus, sonst Probleme mit mod_perl, wenn Configs
...
#
	...
	while ( ($par,$val) = each %{$opt} ) {
		if ( $par eq 'value' ) {
			...
			# Die SRC Option enthält eine Bildreferenz (nur wichtig
			# in non Apache Modi)
			...
			# alle anderen Optionen werden übernommen
			$par =~ tr/A-Z/a-z/;	# schöner so, sacht der Jörn
			if ( $par ne 'sticky' ) {
				$code .= qq[ $par="$val"];
			}
